    ...
    <nav className={`fixed z-40 w-full transition-all duration-300 ${isScrolled ? "py-3 bg-background/80 backdrop-blur-md shadow-xs" : "py-5"}`}>
      <div className="flex justify-between  items-center container">
        ...
        <div className="hidden md:flex space-x-8 mr-4">
          ...
          <button className="bg-primary rounded-xl w-30 hover:bg-primary/90 h-8 shadow-2xl cursor-pointer " onClick={()=>navigate("/login")} >Login</button>
        </div>
  ...
  `}
        >
          <div className="flex flex-col space-y-8 text-xl">
            ...
            <button className="bg-primary rounded-xl w-40 cursor-pointer hover:bg-primary/90 h-8 shadow-2xl " onClick={() => navigate("/login")} >Login</button>
          </div>
        </div>
